...
// IMPORTANT:
// Register ProblemDetails only once.
...
{
    // Angular sends the readable XSRF token using this header.
...
// ============================================================================
//
// Controller creates the transcript status.
// Background worker updates the same status.
//
// Singleton is required so both use the same in-memory store.
//
...
// IMPORTANT:
//
// AddOpenApi() without a name already creates "v1".
// Therefore DO NOT also use:
//      builder.Services.AddOpenApi();
//
// We explicitly create v1 and v2 instead.
//
...
// ============================================================================
//
// AddProblemDetails + UseStatusCodePages lets ASP.NET Core create
// ProblemDetails for responses such as 404, 400, etc. when no body exists.
//
...
// ============================================================================
//
// Must be after routing and before authentication/authorization.
//
// This also supports direct Angular → ASP.NET connections if needed.
//
...
// ============================================================================
//
// Endpoint-specific rate limiting requires routing to have already run.
//
...
// ============================================================================
//
// If the caller is authenticated, create/read an antiforgery token
// and make the request token available to Angular.
//
// NOTE:
// This generates the token.
// It does NOT globally validate SignalR negotiate requests.
//
        ...
        )
        {
            ...
            if (tokens.RequestToken is not null)
            {
                context.Response.Cookies.Append(
                    ...
                    new CookieOptions
                    {
                        // Angular JavaScript needs to read it.
                        HttpOnly = false,

                        // Development is currently using HTTP.
...
//
// Browser:
// localhost:4200/hubs/tms
//
// Angular proxy forwards to:
// localhost:5150/hubs/tms
//
